...
> - `loader()` reading `select * from chat where id = :id and user_id = :userId` → that query lives _directly inside_ the `async` Server Component (`page.tsx`). It awaits the DB and passes the `Message[]` down as a prop. **There is no `loader` export and no `useLoaderData`** — the data is just in scope.
